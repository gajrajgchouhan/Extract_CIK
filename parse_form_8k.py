...
file = os.listdir("./download_filings/sec-edgar-filings/0001118974/8-K")[0]
txt = open("./download_filings/sec-edgar-filings/0001118974/8-K/"+file+"/full-submission.txt", "r").read()

def compile(p,f):
    return re.compile(p,f)

pattern = re.compile(r"<TYPE>8-K.*</TEXT>", re.S)
txt = pattern.finditer(txt)
op = open("./parsed_forms/op.txt", "w")
# for match in txt:
#     print(match)
...
for t in txt:
    txt = t.group()
    break
patterns_for_remove = [compile(i, j) for i, j in patterns_for_remove]
...
json.dump(final, open("./parsed_forms/"+file+".json", "w"), indent=4)
